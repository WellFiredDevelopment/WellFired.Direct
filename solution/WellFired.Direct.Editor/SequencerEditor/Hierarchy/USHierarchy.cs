	...
	public class USHierarchy : ScriptableObject
	{
		...
		public void DoGUI(int depth)
		{
			...
			if(Scrollable)
			{
				...
				using(new Shared.GUIBeginArea(VisibleArea))
				{
					using(new Shared.GUIBeginScrollView(scrollPos, GUIStyle.none, GUIStyle.none))
					{
						using(new Shared.GUIBeginVertical())
						{
							...
							using(new Shared.GUIBeginHorizontal(GUILayout.MaxWidth(FloatingWidth - 5.0f)))
							{
