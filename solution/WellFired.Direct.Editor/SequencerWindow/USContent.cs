...
namespace WellFired
{
	...
	public class USContent : ScriptableObject 
	{
		...
		private bool extendingFloatingWidth;
		private float additionalFloatingWidth;
		...
		public void UpdateCachedMarkerInformation()
		{
			...
			var currentZoomUpper = Mathf.Ceil(ZoomInfo.currentZoom);
			var currentZoomlower = Mathf.Floor(ZoomInfo.currentZoom);
			var zoomRatio = (ZoomInfo.currentZoom - currentZoomlower) / (currentZoomUpper / currentZoomUpper);
	
			var maxDuration = Mathf.Min(CurrentSequence.Duration, 10);
			var minXMarkerDist = ScrubArea.width / maxDuration;
			var maxXMarkerDist = minXMarkerDist * 2.0f;
			ZoomInfo.currentXMarkerDist = maxXMarkerDist * zoomRatio + minXMarkerDist * (1.0f - zoomRatio);
			
			var minXSmallMarkerHeight = ScrubArea.height * 0.1f;
			var maxXSmallMarkerHeight = ScrubArea.height * 0.8f;
			var currentXSmallMarkerHeight = maxXSmallMarkerHeight * zoomRatio + minXSmallMarkerHeight * (1.0f - zoomRatio);
			
			// Calculate our maximum zoom out.
			var maxNumberOfMarkersInPane = ScrubArea.width / minXMarkerDist;
			ZoomInfo.meaningOfEveryMarker = Mathf.Ceil(CurrentSequence.Duration / maxNumberOfMarkersInPane);
	
			var levelsDeep = Mathf.FloorToInt(ZoomInfo.currentZoom);
			while(levelsDeep > 1)
			{
			...
			// Create our markers
			var markerValue = 0.0f;
			var markerRect = new Rect(-ScrollInfo.currentScroll.x, 0.0f, 1.0f, maxXSmallMarkerHeight);
			while(markerRect.x < ScrubArea.width)
			{
				...
				for(var n = 1; n <= 10; n++)
				{
					var xSmallPos = markerRect.x + ZoomInfo.currentXMarkerDist / 10.0f * n;
					
					var smallMarkerRect = markerRect;
