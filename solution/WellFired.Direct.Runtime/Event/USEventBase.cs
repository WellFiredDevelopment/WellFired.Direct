	...
	abstract public class USEventBase : MonoBehaviour 
	{
		...
		public USTimelineBase Timeline
		{
			get
			{
				...
				var timeline = transform.parent.GetComponent<USTimelineBase>();
		...
		public void FixupAdditionalObjects()
		{
			...
			var additionalObjects = new List<Transform>();
			foreach(var objectPath in serializedAdditionalObjectsPaths)
			{
				var additionalObject = GameObject.Find(objectPath);
				if(additionalObject)
