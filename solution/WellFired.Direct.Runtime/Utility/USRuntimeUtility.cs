...
namespace WellFired
{
	...
	public static class USRuntimeUtility 
	{
		...
		static public void CreateAndAttachObserver(USSequencer sequence)
		{
			...
			var spawnedContainer = new GameObject("_Timelines for Observer");
			spawnedContainer.transform.parent = sequence.transform;
			var ourTimelineContainer = spawnedContainer.AddComponent(typeof(USTimelineContainer)) as USTimelineContainer;
			ourTimelineContainer.Index = -1;
			
			var spawnedObserver = new GameObject("Observer");
		...
		{
			var recorder = Object.FindObjectOfType(typeof(USRecordSequence)) as USRecordSequence;
		...
		{
			var keyframeTime = sequence.Duration;
			foreach(var timelineContainer in sequence.TimelineContainers)
			{
				foreach(var timelineBase in timelineContainer.Timelines)
				{
					...
					foreach(var property in timelineProperty.Properties)
					{
						...
						{
							var time = curve.FindNextKeyframeTime(sequence.RunningTime);
