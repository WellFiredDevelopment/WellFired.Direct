...
namespace WellFired
{
	...
	public class USPropertyInfo : ScriptableObject
	{
		...
		[SerializeField] private long		baseLong;
		[SerializeField] private float 		baseFloat;
		[SerializeField] private double		baseDouble;
		[SerializeField] private bool 		baseBool;
		...
		public PropertyInfo propertyInfo
		{
			get 
			{
				...
				cachedPropertyInfo = Shared.PlatformSpecificFactory.ReflectionHelper.GetProperty(type, propertyName);
				return cachedPropertyInfo;
			}
			set
			{
		...
		public FieldInfo fieldInfo
		{
			get 
			{
				...
				var type = component.GetType();
				cachedFieldInfo = Shared.PlatformSpecificFactory.ReflectionHelper.GetField(type, fieldName);
				return cachedFieldInfo;
			}
			set
			{
		...
		public void SetValue(float time)
		{
			...
			var earliestTime = Mathf.Infinity;
			var latestTime = 0.0f;
			...
			var wasPreviouslyInCurveArea = false;
			var isCurrentlyInCurveArea = false;
			var didPlayPassCurveArea = false;
			var didPlayPassCurveAreaInReverse = false;
			var isUseCurrentValue = false;
			...
			for(var curveIndex = 0; curveIndex < curves.Count; curveIndex++)
			{
				...
				var firstKeyframeTime = curves[curveIndex].FirstKeyframeTime;
				var isInitialKeyframe = previousTime < firstKeyframeTime && time >= firstKeyframeTime;
			...
			case PropertyTypeInfo.Bool:
				var bestValue = false;
				foreach(var keyframe in curves[0].Keys)
				{
		...
		public void AddKeyframe(List<USInternalCurve> settingCurves, object ourValue, float time, CurveAutoTangentModes tangentMode)
		{
			...
			case PropertyTypeInfo.Vec2:
				{
					var newVector2 = (Vector2)ourValue;
			...
			case PropertyTypeInfo.Vec3:
				{
					var newVector3 = (Vector3)ourValue;
					tmpKeyframe.time = time;
			...
			case PropertyTypeInfo.Vec4:
				{
					var newVector4 = (Vector4)ourValue;
					tmpKeyframe.time = time;
			...
			case PropertyTypeInfo.Quat:
				{
					var newQuat = (Quaternion)ourValue;
					tmpKeyframe.time = time;
		...
		{
			var internalKeyframe = curve.AddKeyframe(keyframe.time, keyframe.value);
		...
		{	
			...
			{
			case PropertyTypeInfo.Int:
				curves.Add(CreateInstance<USInternalCurve>());
