...
namespace WellFired
{
	...
	public class USTimelineAnimationGameRunner : ScriptableObject
	{
		...
		{
			for(var clipIndex = 0; clipIndex < allClips.Count; clipIndex++)
			{
		...
		private void SanityCheckClipData()
		{
			...
			{
				var inNotRunning = notRunningClips.Contains(clip);
				var inRunning = runningClips.Contains(clip);
				var inFinished = finishedClips.Contains(clip);
