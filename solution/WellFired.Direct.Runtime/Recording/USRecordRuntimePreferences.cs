...
namespace WellFired
{
	public static class USRecordRuntimePreferences
	{
		...
		{
			var last = Application.dataPath.LastIndexOf("/");
			var defaultCapturePath = Application.dataPath.Remove(last);
		...
		{
			var presetInfo = new List<PresetInfo>();
			...
			{
				var presetsSplitData = presetsData.Split(')');
				foreach(var presetData in presetsSplitData)
				{
					...
					var data = presetData.Replace("(", "").Replace(")", "");
					var presetParams = data.Split(';');
					var presetName = presetParams[0];
					var savePath = presetParams[1];
					var presetUpscaleValue = (USRecord.Upscaling)Enum.Parse(typeof(USRecord.Upscaling), presetParams[2]);
					var presetFramerate = (USRecord.FrameRate)Enum.Parse(typeof(USRecord.FrameRate), presetParams[3]);
		...
		public static void SaveAsNewPreset(string name, string capturePath, USRecord.Upscaling upscaleAmount, USRecord.FrameRate frameRate)
		{
			...
			// presetData is defined as such
			// (Default;c:/Blah/Save_Loc;_2;_60)
			var preset = String.Format("({0};{1};{2};{3})", name, capturePath, upscaleAmount, frameRate);
			Presets += preset;
		}
		...
		private static string UniqifyName(string name)
		{
			var unique = true;
			var presetsList = GetPresetInfo();
			foreach(var info in presetsList)
			{
				if(info.Name == name)
					unique = false;
			}
