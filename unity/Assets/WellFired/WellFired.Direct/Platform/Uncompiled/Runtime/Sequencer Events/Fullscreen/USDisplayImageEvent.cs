...
namespace WellFired
{
	...
	public class USDisplayImageEvent : USEventBase 
	{
		...
		{
			var alpha = fadeCurve.Evaluate(fadeCurve.keys[fadeCurve.length - 1].time);
		...
		{	
			...
			var maxTime = 0.0f;
			foreach(var key in fadeCurve.keys)
			{
				if(key.time > maxTime)
					maxTime = key.time;
			}
			...
			var alpha = fadeCurve.Evaluate(currentCurveSampleTime);
			alpha = Mathf.Min(Mathf.Max(0.0f, alpha), 1.0f);
			
			if(!displayImage)
				return;
			
			var position = new Rect(Screen.width * 0.5f, Screen.height * 0.5f, displayImage.width, displayImage.height);
